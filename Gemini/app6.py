...
OUTPUT_FILE = "results/non-creamyLayer_2_gemini_result.json"
...
def extract_with_gemini(file_path):
    ...
    logger.info("=" * 60)
    logger.info(
        "DIRECT GEMINI NON-CREAMY LAYER CERTIFICATE EXTRACTION STARTED"
    )
    logger.info("=" * 60)
    ...
    prompt = """
Analyze the provided Non-Creamy Layer certificate image.
...
"""
    ...
    logger.info(
        "Preparing certificate image for Gemini..."
    )
    ...
    logger.info("-" * 60)
    logger.info(
        "SENDING NON-CREAMY LAYER CERTIFICATE DIRECTLY TO GEMINI"
    )
    logger.info("-" * 60)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(
        "NON-CREAMY LAYER CERTIFICATE DIRECT GEMINI TEST"
    )
    logger.info("=" * 60)
