...
The document is an Indian DOMICILE CERTIFICATE
also known as a RESIDENCE / DOMICILE CERTIFICATE.
...
- Do not infer a date that is not present.
- Do not infer a validity period.
- Do not create a certificate number.
- Do not create an application number.
...
- Preserve Aadhaar numbers exactly as written.
- Do not reconstruct missing Aadhaar digits.
- Do not confuse the applicant address with the
  issuing authority's office address.
- Extract village, taluka, district and state separately
  whenever they are explicitly available.
- If the certificate states that the applicant is a
  resident/domicile of a particular state, extract the
  corresponding domicile status.
- If the certificate contains residence since a year/date,
  extract it.
- If the certificate explicitly states the number of years
  of residence, extract it.
- Do not calculate years of residence.
- Extract issuing authority and designation separately.
- If there is a QR code but its contents are not present
  in OCR text, do not invent its data.
...
Domicile/residence may appear as:
- Domicile
- Residence
- Resident of
- Permanent Resident
...
Extract the actual person's or authority's information.
...
def main():
    ...
    logger.info("=" * 70)
    logger.info(
        "DOMICILE CERTIFICATE EXTRACTION PIPELINE STARTED"
    )
    logger.info("=" * 70)
    ...
    try:
        ...
        print("")
        print("=" * 70)
        print("FINAL DOMICILE CERTIFICATE JSON")
        print("=" * 70)
        ...
        logger.info(
            "DOMICILE CERTIFICATE EXTRACTION "
        ...
        logger.exception(
            "Domicile certificate extraction failed: %s",
            error
        )
